        ...
        switch(SetupPkt.bDescriptorType)
        {
            ...
            case USB_DESCRIPTOR_STRING:
                ...
                if(SetupPkt.bDscIndex<USB_NUM_STRING_DESCRIPTORS)
                {
                    ...
                    const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[1];}sda00={
                    sizeof(sda00),USB_DESCRIPTOR_STRING,{0x0409}};

                    //Manufacturer string descriptor
                    const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[10];}sda01={
                    sizeof(sda01),USB_DESCRIPTOR_STRING,
                    {'T','E','C','N','O','D','A','L','V','O'}
                    };

                    //Product string descriptor
                    const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[4];}sda02={
                    sizeof(sda02),USB_DESCRIPTOR_STRING,
                    {'T','D','u','C'}
                    };
                    
                    const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[12];}sda03={
                        sizeof(sda03),USB_DESCRIPTOR_STRING, 
                        {SN.string[0],SN.string[1],SN.string[2],SN.string[3],SN.string[4],SN.string[5],
                        SN.string[6],SN.string[7],SN.string[8],SN.string[9],SN.string[10],SN.string[11]}
                    };
                    
                    //Array of string descriptors
                    const uint8_t *const USB_SD_Ptr2[USB_NUM_STRING_DESCRIPTORS]=
                    {
                        (const uint8_t *const)&sda00,
                        (const uint8_t *const)&sda01,
                        (const uint8_t *const)&sda02,
                        (const uint8_t *const)&sda03
                    };
                    
                    inPipes[0].pSrc.bRom = *(USB_SD_Ptr2+SetupPkt.bDscIndex);
                    // Set data count
